...
class document(Root, Structural, Element):
    ...
    def _add_suffix_for_id(self, prefix):
        i = 1
        id = ''
        ...
        if not node['ids']:
            ...
            for name in node['names']:
                ...
                if id and id in self.ids:
                    id = self._add_suffix_for_id(id)
                    break
            for name in node['dupnames']:
                id = self.settings.id_prefix + make_id(name)
                if id and id not in self.ids:
                    break
                if id and id in self.ids:
...
def make_id(string):
    ...
    """
    id = string
    id = _non_ascii_word_bondary.sub("-", id)
    id = unidecode.unidecode(id)
    id = id.lower()
...
_non_ascii_word_bondary = re.compile(r'(?<=[a-zA-Z0-9])\B(?=[^a-zA-Z0-9])|(?<=[^a-zA-Z0-9])\B(?=[a-zA-Z0-9])')
_non_id_translate = {
